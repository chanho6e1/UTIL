...
const GoalDetailRTilItem = (props) => {
  ...
  return (
    <Link className={classes["goal-detail-r-tils-item-link"]} to={`/index/post/${props.til.postId}`}>
      <div className={classes["goal-detail-r-tils-item-title"]}>{props.til.title}</div>
    </Link>
  );
};
