...
const PlanCalendar = (props) => {
  ...
  const [monthDistance, setMonthDistance] = useState(
    getMonthDistance(props.startRange, props.endRange) + 1
  );
  const [monthRange, setMonthRange] = useState(
    Array(monthDistance)
      ...
      .map((arr, idx) => {
        return new Date(
          props.startRange.getFullYear(),
          props.startRange.getMonth() + idx + 1,
          0
        );
      })
  );
  const planGridRef = Array(Object.keys(plans).length).fill(
    useRef([]),
    0,
  ...
  useEffect(() => {
    ...
    if (props.columns && dateRangeWrapperRef?.current?.style?.height) {
      dateRangeWrapperRef.current.style.height =
        (props.columns + 1) * (planGridRef[0].current[0].clientHeight + 2) +
        "px";
    }
  ...
  const monthGridDummy = monthRange.map((el, idx) => {
    return (
      <div key={`month-plan-bar-${idx}`}>
        <div
          id={`month-${props.startRange.getMonth() + idx + 1}`}
          className={`${styles["month-plan-bar"]} ${
            plans.length % 2
              ? styles["container-odd"]
              : styles["container-even"]
          }`}
  ...
  const gridPerPlans = (rowIdx) => {
    const exec = monthRange.map((el, idx) => {
      return (
        <div key={`month-plan-bar-${idx}`}>
          <div
            ref={(el) => (planGridRef[rowIdx].current[idx] = el)}
            id={`month-${props.startRange.getMonth() + idx + 1}`}
            className={`${styles["month-plan-bar"]} ${
              rowIdx % 2 ? styles["container-odd"] : styles["container-even"]
            }`}
            style={{
              width: `${props.contracted === true ? "170px" : "250px"}`,
            }}
          ></div>
        </div>
      );
  ...
  const monthTitleGrid = monthRange.map((el, idx) => {
    ...
    return (
      <div key={`month-title-bar-${idx}`}>
        <div
          className={styles["month-title-bar"]}
          style={{ width: `${props.contracted === true ? "170px" : "250px"}` }}
        >
          {tempMonth.getFullYear()}년 {tempMonth.getMonth() + 1}월
        </div>
      </div>
    );
  });
      ...
      <React.Fragment key={`month-bar-container-${plans[el].goalId}`}>
        <div ref={containerRef} className={styles["month-bar-container"]}>
          ...
          <PlanCalendarDateSelector
            ...
            gridStart={props.startRange}
            gridEnd={props.endRange}
            extendStartRange={props.extendStartRange}
            extendEndRange={props.extendEndRange}
          />
        </div>
        {props.todoFormVisibility[idx] && (
          <PlanTodoListRight
            ...
            newTodoGoalId={props.newTodoGoalId}
          />
        )}
      </React.Fragment>
    );
  ...
  const newPlanDummy = (
    <div className={styles["new-plan-dummy"]}>{monthGridDummy}</div>
  );

  const syncScroll = () => {
    monthTitleWrapperRef.current.style.left =
      -scrollRef.current.children[1].scrollLeft + "px";
    props.plansTitleWrapperRef.current.style.top =
      -scrollRef.current.children[1].scrollTop + "px";
    if (scrollRef.current.children[1].scrollTop !== 0) {
      monthTitleWrapperRef.current.style.boxShadow =
        "0px 2px 10px 0px rgb(77, 71, 71, 0.2)";
    } else {
      monthTitleWrapperRef.current.style.boxShadow = "none";
    }
    if (scrollRef.current.children[1].scrollLeft !== 0) {
      props.plansTitleInnerRef.current.style.boxShadow =
        "2px 0px 10px 0px rgb(77, 71, 71, 0.2)";
    } else {
  ...
  return (
    <div
      id="date-range"
      ref={dateRangeWrapperRef}
      className={styles["date-range-wrapper"]}
    >
      <div
        ref={monthTitleWrapperRef}
        className={styles["month-title-container"]}
        style={{ width: `${containerRef?.current?.scrollWidth}px` }}
      >
        {monthTitleGrid}
      </div>
