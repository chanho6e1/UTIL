    ...
    return (
      <React.Fragment key={`month-bar-container-${plans[el].goalId}`}>
        <div ref={containerRef} className={styles['month-bar-container']} >
          {columns}
          <PlanCalendarDateSelector idx={idx} plan={plans[el]} todos={props.todos} startDate={plans[el].startDate} endDate={plans[el].endDate} planGridRef={planGridRef} xPointLib={xPointLib} monthRange={monthRange} gridStart={props.startRange} gridEnd={props.endRange} extendStartRange={props.extendStartRange} extendEndRange={props.extendEndRange} />
          
        </div>
        {props.todoFormVisibility[idx] && <PlanTodoListRight applyTodoData={props.applyTodoData} getInputTodoData={props.getInputTodoData} plan={plans[el]} goalId={plans[el].goalId} todos={props.todos} scrollRef={scrollRef} containerRef={containerRef} newTodoIdx={props.newTodoIdx} />}
      </React.Fragment>
    )
  })
