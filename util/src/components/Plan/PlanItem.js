...
const PlanItem = (props) => {
  ...
  const titleReadMode = (
    <div onClick={enterEditMode} className={styles["plan-title-bar-button"]}>
      <div className={styles["plan-title"]}>{props.plan.title}</div>
  ...
  const arrowRender = (
    <div
      onClick={props.todoFormToggleHandler.bind(
        this,
        props.idx,
        props.plan.goalId
      )}
      className={styles["plan-title-bar-icon-wrapper"]}
    >
      <img
        className={styles["arrow-icon"]}
        src={arrow}
        style={{
          transform: props.todoFormVisibility[props.idx]
            ? "rotate(90deg)"
            : "none",
          marginLeft: "13px",
          width: "12px",
  ...
  return (
    <React.Fragment>
      ...
      />
      <div
        id={`${props.plan.goalId}`}
        className={`${styles["plan-title-bar"]} ${
          props.idx % 2 ? styles["title-odd"] : styles["title-even"]
        }`}
