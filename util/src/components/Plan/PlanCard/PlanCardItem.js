...
const PlanCardItemForm = (props) => {
  ...
  const todoList = todos && todos.map((todo, idx) => {
    return (
      <PlanCardItemTodo key={`plan-card-${todo.todoId}-${props.today === true ? 'today' : 'per-plan'}`} todo={todo} plan={props.today === true ? plans[todo.goalId] : props.plan} className={idx % 2 ? 'odd' : 'even'} today={props.today} />
    )
  })
  ...
  const empty = (
    <div style={{height:`${48 * 3}px`, display:'flex', justifyContent:'center', alignItems:'center'}}>
        TODO를 생성해 주세요.
    </div>
  )
  ...
  const headerButtons = (
    <div className={styles['button-wrapper']}>
      <div onClick={() => {setPlanModalState(true)}} className={styles['new-button']}>
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-pencil-square" viewBox="0 0 16 16">
          <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
          <path fillRule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
        </svg>
      </div>
      <div onClick={() => {setTodoModalState(true)}} className={styles['new-button']}>
  ...
  return (
    <Card className={styles['plan-card']}>
      <FixedModal modalState={todoModalState} stateHandler={setTodoModalState} content={<PlanCardTodoCRUD plan={props.plan} />} width={'400px'} height={'auto'} />
      <FixedModal modalState={planModalState} stateHandler={setPlanModalState} content={<PlanCardPlanCRUD plan={props.plan} />} width={'400px'} height={'auto'} />
      <header className={styles['header']}>
        <div className={styles['plan-title-wrapper']}>
          <span>{props.today === true ? '오늘 할 일' : props?.plan?.title}</span>
      ...
      <div className={styles['todos-wrapper']} ref={scrollRef}>
        <div className={styles['todos-inner-wrapper']}>
          {todoList}
        </div>
