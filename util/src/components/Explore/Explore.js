...
const ExploreForm = () => {
  ...
  useEffect(() => {
      setIsLoading(() => true)
      criteriaAPI[criteria](criteriaList[criteria], 1, 20)
        .then((res) => {
  ...
  const fetchMoreData = () => {
    setIsLoading(() => true)
    criteriaAPI[criteria](criteriaList[criteria], offset + 1, size)
      .then((res) => {
  ...
  const altMessages = (
      <Fragment>
        {(criteria !== 0 && myTagList.length === 0) && <div className={classes['alt-message-wrapper']}>{message1}</div>}
        {(criteria !== 0 && feedList.length === 0) && <div className={classes['alt-message-wrapper']}>{message2}</div>}
        {(criteria === 0 && feedList.length === 0) && <div className={classes['alt-message-wrapper']}>{message3}</div>}
      </Fragment> 
  )
