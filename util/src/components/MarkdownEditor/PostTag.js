...
const PostTag = (props) => {
    ...
    return (
      <div key={`line-${idx}`} className={styles[`line-wrapper`]}>
        <div
          ...
          onInput={inputToArray.bind(this, idx)}
          onKeyDown={(event) => {
            deleteLine(idx, event);
            pushLine(idx, event);
            preventSpace(idx, event);
          }}
        ></div>
      </div>
    );
  ...
  const loadCursorLocation = async (
    line,
    leftContent = "",
    rightContent = ""
  ) => {
  ...
  const holdCursorLocation = async (
    line,
    leftContent = "",
    rightContent = ""
  ) => {
  ...
  const deleteLine = async (line, event) => {
    ...
    if (
      selection.anchorOffset === frontData[line].length &&
      event.key == "Delete"
    ) {
