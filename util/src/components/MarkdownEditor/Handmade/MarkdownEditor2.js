...
const MarkdownEditor = (props) => {
  ...
  const preventDeletingFirstLine = (event) => {
    if (
      ...
      lineRef.current.previousSibling.id !== lineRef.current.id &&
      event.key === "Backspace"
    ) {
    ...
    const reactImage = (
      <React.Fragment>
        <div class={styles["wrapper-block"]} contentEditable="true">
          ...
          >
            <img
              ref={(el) => (imageRef.current[order] = el)}
              src={url}
              id={`image-${order}`}
              alt={url}
            ...
            />
            <div
              ref={(el) => (imageUrlRef.current[order] = el)}
              id={`image-url-${order}`}
            >{`![](${url})`}</div>
          </div>
        </div>
  ...
  return (
    <div className={styles.markdown}>
      ...
      <div
        ...
        onClick={giveRefHandler}
        onKeyDown={preventDeletingFirstLine}
        contentEditable="true"
      ...
      <div>
        {editorWrapperRef.current && (
          <Example2 content={editorWrapperRef.current.innerText} />
        )}
      </div>
    </div>
  );
