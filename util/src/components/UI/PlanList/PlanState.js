...
const PlanState = (props) => {
  ...
  const nowTime = () => {
    let now = new Date
    let nowM = (now.getMonth() + 1) > 9 ? (now.getMonth() + 1) : '0' + (now.getMonth() + 1) ;
    let nowD = now.getDate() > 9 ? now.getDate() : '0' + now.getDate() ;
    return (`${now.getFullYear()}-${nowM}-${nowD}`)
  }

  const isComplete = (props.state
    ?
    planComplete
    :
    planUnComplete
  ...
  const isRunning = (nowTime() <= props.startDate
    ?
    plancomming
    :
    planRunning
  ...
  const nowState = (nowTime() > props.endDate
    ?
    isComplete
    :
    isRunning
