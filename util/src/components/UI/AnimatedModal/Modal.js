...
const ModalOverlay = (props) => {
  ...
  const mountModalHandler = async () => {
    const modalInitialize = async () => {
      ...
      contentRef.current.style.top =
        (await props.parentRef.current.getBoundingClientRect().top) + "px";
      contentRef.current.style.width =
        (await props.parentRef.current.clientWidth) + "px";
      contentRef.current.style.height =
        (await props.parentRef.current.clientHeight) + "px";
    };
    ...
    const modalExecute = () => {
      ...
      const responsiveWidth =
        (100 * indicatorRef.current.clientWidth) / window.innerWidth;
      const responsiveHeight =
        (100 * indicatorRef.current.clientHeight) / window.innerHeight;
      ...
      contentRef.current.style.left =
        indicatorRef.current.getBoundingClientRect().left + "px";
      contentRef.current.style.top =
        indicatorRef.current.getBoundingClientRect().top + "px";
    };
  ...
  const unmountModalHandler = async () => {
    ...
    const modalExecute = async () => {
      ...
      contentRef.current.style.left =
        (await indicatorRef.current.getBoundingClientRect().left) + "px";
      contentRef.current.style.top =
        (await indicatorRef.current.getBoundingClientRect().top) + "px";
  ...
  const modal =
    ...
    ReactDOM.createPortal(
      <ModalOverlay
        ...
        toggleBoolean={props.toggleBoolean}
        url={props.url}
