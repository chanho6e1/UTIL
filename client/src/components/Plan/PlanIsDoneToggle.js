...
const PlanIsDoneToggle = (props) => {
  ...
  const toggleIsDone = () => {
    chkTodoAPI(props.todo.todoId, props.plan.goalId)
      ...
      .then((res) => {
        ...
        recvIsAllTodosDoneAPI(props.plan.goalId).then((res) => {
          if (res === true) {
  ...
  const redirectToPost = (isPlanDone) => {
    const url = `/create/post?goal_id=${props.plan.goalId}${
      isPlanDone ? "&ask_done=true" : ""
    }&step=true`;
    navigate(url);
  };
  ...
  const message1 = (
    ...
    >
      <div>
        <b>{props.todo.title}</b>을 완료하였습니다.
      </div>
  ...
  const isDoneTrue = (
    <div
      ...
      className={styles["is-done-true-wrapper"]}
    >
      완료됨
    </div>
  );
  ...
  return (
    <React.Fragment>
      <FixedModal
        ...
        content={
          <DoneModalForm redirectToPost={redirectToPost} planDone={planDone} />
        }
        width={"350px"}
        height={"auto"}
      />
