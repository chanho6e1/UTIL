...
public interface TodoRepository extends JpaRepository<Todo, Long> {
    ...
    @Query("SELECT t FROM Todo t join Goal g on t.goal.goalId = g.goalId WHERE t.dueDate = :dueDate and g.user.userId = :userId and g.state = false ")
    List<Todo> findByDate(@Param("userId") Long userId, @Param("dueDate") String dueDate);
    ...
    @Query("select g " +
            "from Goal g " +
            "where g.goalId = :goal and :dueDate between g.startDate and g.endDate ")
    Optional<Goal> checkTodoAndGoalPeriod(@Param("goal") Long goal, @Param("dueDate") String dueDate);
